...
for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
  ...
  await page.goto(`${base}/`, { waitUntil: "networkidle" });
  await page.getByRole("link", { name: "Endpoint & Protokoll" }).click();
  await page.waitForURL(/\/service-contract$/);
  check((await page.locator("main").innerText()).toLocaleLowerCase("de").includes("nicht belegt"), `${viewport.width}px: contract discovery failed`);
